-- ===========================================================================
-- 0011: Starting smart practice always builds a fresh session.
--
-- start_smart_practice_session used to resume the newest still-'active'
-- session whenever it had a presentable pending card (0007 only retired
-- *dead* sessions). Nothing in the app ever abandons a session -- the only
-- way out of 'active' is answering every queued card -- so after leaving a
-- round unfinished, every home session button (Auto / New material / Review)
-- silently reopened that same queue: the same cards, in the old session's
-- mode, no matter which focus was requested.
--
-- Fix: pressing a session button means "start a round now with these
-- settings" (PracticePage's mount is this RPC's only caller). Retire any
-- lingering active sessions as 'abandoned' and always build a new queue:
--   * new cards    -- uniform random pick from unmastered material (unchanged)
--   * review cards -- due-first, lowest predicted recall first, topped up
--                     with the soonest-due (unchanged)
-- Abandoning loses nothing: per-card FSRS progress is written on every swipe,
-- and still-due cards are simply picked first again by the next queue. This
-- also supersedes the 0007 dead-session resume guard.
-- ===========================================================================
