-- Let review sessions start from as few as 10 cards instead of 20, so users
-- with a small due queue aren't forced to pad it out to a bigger batch size.
-- Mirrors the new REVIEW_BATCH_SIZE_RANGE.min in HomePage.jsx.
